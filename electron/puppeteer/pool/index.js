 ...
 * @param {Number} [options.min=5] 保证池中最少有多少个实例存活
 * @param {Number} [options.maxUses=20] 每一个 实例 最大可重用次数，超过后将重启实例。0表示不检验
 * @param {Number} [options.testOnBorrow=true] 在将 实例 提供给用户之前，池应该验证这些实例。
 * @param {Boolean} [options.autostart=true] 是不是需要在 池 初始化时 初始化 实例
 ...
 * @param {Function} [options.validator=(instance)=>cnashu .resolve(true))] 用户自定义校验 参数是 取到的一个实例
...
const initPuppeteerPool = (options = {}) => {
  const {
    ...
    testOnBorrow = true,
    autostart = true,
    idleTimeoutMillis = 1200000, // 20min 闲置重启
    evictionRunIntervalMillis = 600000, // 10min 检查状态
    puppeteerArgs = {},
    ...otherConfig
  } = options;
  let launchList = 0;
  const factory = {
    ...
    validate: instance => {
      // const pageLength = (await instance.pages()).length;
      // const outLnegth = pageLength < 2;
      const isReslove = maxUses <= 0 || instance.useCount < maxUses;
      return Promise.resolve(isReslove);
    },
  };
  const config = {
    max,
